...
(function () {
  ...
  const SCREENS_CONFIG = {
    ...
    2: { name: 'Ресторан LenMar', image: 'Photo_screens/2 - Restaurant_new.webp?v=101' },
    3: { name: 'SPA & Басейн', image: 'Photo_screens/3 - Spa.webp?v=101' },
  ...
  function getTransitionVideoSrc(from, to) {
    if (isMobileViewport()) {
      return `Photo_screens/mob_video/${from}-${to}.mp4?v=77`;
    }
    return `Photo_screens/${from}-${to}.mp4?v=77`;
  }
