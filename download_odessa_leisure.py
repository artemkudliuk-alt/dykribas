...
dest_dir = "e:/ribas_Dyk/screens_photos/screen_6_leisure_odessa"
for it in nested:
    p = it.get("parent_folder", "")
    fname = it.get("text", "")
    if it.get("isFolder") or not fname.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
        continue
    if any(k in p.lower() for k in ["одесса", "одеса", "ugc", "giardini"]):
        clean_p = p.split("/")[-1].strip().replace(" ", "_")
        dest = os.path.join(dest_dir, f"{clean_p}__{fname}".replace(" ", "_"))
        download_file(it["id"], dest)

print("Odessa / Leisure download complete.")
