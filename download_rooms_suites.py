...
rooms_dir = "e:/ribas_Dyk/screens_photos/screen_5_rooms_pillows"
for it in nested:
    p = it.get("parent_folder", "")
    fname = it.get("text", "")
    if it.get("isFolder") or not fname.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
        continue
    if any(k in p.lower() for k in ["люкс", "напівлюкс", "302", "202", "подушки", "301", "304"]):
        clean_p = p.split("/")[-1].strip().replace(" ", "_")
        dest = os.path.join(rooms_dir, f"{clean_p}__{fname}".replace(" ", "_"))
        download_file(it["id"], dest)

print("Rooms download done.")
